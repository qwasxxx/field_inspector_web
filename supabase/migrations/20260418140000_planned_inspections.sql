-- Planned inspections: schedule future patrols; dispatch creates inspection_tasks + items + assignment.
-- Dispatch: call public.dispatch_planned_inspections_due() from Edge Function (service_role) or admin UI (RPC).
--
-- public.is_admin(uuid) обычно создаётся в миграции inspection_tasks; если её не запускали — создаём здесь.
...
-- ---------------------------------------------------------------------------
-- Dispatch due plans into real tasks (SECURITY DEFINER; allowed: service_role or admin JWT)
-- ---------------------------------------------------------------------------
